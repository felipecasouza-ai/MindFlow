...
interface HeaderProps {
  onViewChange: (view: 'library' | 'dashboard' | 'stats' | 'admin' | 'settings') => void;
  user: User | null;
  onLogout: () => void;
        ...
        >
          <div className="w-10 h-10 group-hover:scale-110 transition-transform duration-300">
            <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
              ...
              <path d="M15 40 Q 50 15 85 40" stroke="white" strokeWidth="6" fill="none" strokeLinecap="round" />
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {user && (
            <nav className="flex items-center gap-1 sm:gap-2 mr-2">
              <button onClick={() => onViewChange('library')} className="text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium px-3 py-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-800 transition-colors text-sm">Biblioteca</button>
              ...
              {activePlan && !isAdmin && (
                <>
                  <button onClick={() => onViewChange('dashboard')} className="text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium px-3 py-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-800 transition-colors text-sm">Painel</button>
                  <button onClick={() => onViewChange('stats')} className="text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium px-3 py-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-800 transition-colors text-sm">Estatística</button>
                </>
              )}
            </nav>
          )}
          ...
          <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-800 pl-4">
            <button 
              ...
              className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition-all active:scale-90"
            >
              <ThemeIcon />
            </button>
            ...
            {user && (
              <>
                ...
                <button onClick={onLogout} className="p-2 text-slate-400 hover:text-red-500 rounded-lg transition-all" title="Sair">
